...
Simulates a Hodgkin-Huxley style action potential using:
- Voltage-gated Na+ channels (activation m, inactivation h)
- Voltage-gated K+ channels (activation n)
- Leak current omitted for simplicity
...
# === Plotting ===
fig, ax = plt.subplots(9, 1, figsize=(10, 13), sharex=True)
...
ax[5].set_ylabel('g_Na')

for i, m in enumerate(m_traces):
    ax[6].plot(t, m, color=colors[i])
ax[6].set_ylabel('Na+ m(t)')

for i, h in enumerate(h_traces):
    ax[7].plot(t, h, color=colors[i])
ax[7].set_ylabel('Na+ h(t)')

for i, n in enumerate(n_traces):
    ax[8].plot(t, n, color=colors[i])
ax[8].set_ylabel('K+ n(t)')
